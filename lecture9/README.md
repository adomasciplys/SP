...
2. Start with [meta_test.cpp](src/sum.cpp): uncomment static assertions and implement the corresponding meta predicates until all assertions pass.
...
- In my Printer design, runtime polymorphism scales with node types: I currently have 6 visit(...) overloads (const, var, unary, binary, assign, expr)
- For each new AST I need to add 1 more visit overload in the visitor plus 1 accept override in that node class. 
- So the maintenance cost grows roughly as O(number\_of\_node\_types) in both visitor and node hierarchy.
- `json_ostream` has one main generic output function (`operator<<`) that handles almost all types at compile time. 
- For a new type, you usually do \*not\* write a new overload; you just add `accept(json_writer&)` in that type and list its fields there. 
- This keeps the serializer code mostly unchanged, so maintenance stays low as types grow.

5. Implement support for associative containers like [std::map](https://en.cppreference.com/w/cpp/container/map.html), [std::unordered_map](https://en.cppreference.com/w/cpp/container/unordered_map.html), etc. -- any containers containing [std::pair](https://en.cppreference.com/w/cpp/utility/pair.html)s. First, adjust and add tests for meta-predicates and then implement the functionality. For example, `jos << std::map{{"one",1},{"two",2}}` should produce `{"one":1,"two":2}`. 

6. Implement the input support ([json_input.hpp](include/json_input.hpp), similar to [json_output.hpp](include/json_output.hpp)) to satisfy tests in [json_input_test.cpp](src/json_input_test.cpp).
